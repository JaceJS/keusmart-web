...
import { UserPlus, AlertCircle, Crown, User } from "lucide-react";
import {
  useUsers,
  useInviteMember,
  TeamMemberList,
...
export default function TeamPage() {
  ...
  const { inviteMember, isLoading: isInviting } = useInviteMember(refetch);
  ...
  const handleInvite = async (inviteData: { name: string; email: string }) => {
    try {
      await inviteMember(inviteData);
      setIsInviteModalOpen(false);
      success(
        "Undangan Terkirim",
        `Undangan telah dikirim ke ${inviteData.email}`,
      );
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Undangan gagal dikirim.";
      showError("Gagal Mengirim", message);
    }
  };
  ...
  const handleRemoveConfirm = async () => {
    ...
    setIsRemoving(true);
    try {
      await userService.removeMember(memberToRemove.id);
      refetch();
      success(
        "Anggota Dihapus",
        `${memberToRemove.name} telah dihapus dari tim.`,
      );
    ...
    <>
      <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-border shadow-sm p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900">
                {isLoading
                  ? "..."
                  : data.filter((m) => m.role === "owner").length}
              </p>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-border shadow-sm p-4 flex items-center gap-3">
        ...
        <ConfirmationModal
          ...
          title="Hapus Anggota?"
          message={`Apakah Anda yakin ingin menghapus ${memberToRemove?.name || ""} dari tim?`}
          confirmText="Hapus"
          cancelText="Batal"
          variant="danger"
